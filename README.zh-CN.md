...
<p align="center">
  <strong>面向 JitAI 生态的 OpenClaw 桌面应用</strong>
</p>

<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-clawx">为什么选择 JitClaw</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**JitClaw** 是融入 JitAI 生态的 OpenClaw 桌面应用，为 OpenClaw Agent 提供图形化界面，支持企业账号登录与 JitNode Skills 集成，让原本依赖命令行的 AI 编排能力可以直接在本地桌面中使用。
...
构建 AI 智能体不应该需要精通命令行。JitClaw 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
JitClaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，JitClaw 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
JitClaw 现在还内置了腾讯官方个人微信渠道插件，可直接在 Channels 页面通过内置二维码流程完成微信连接。
...
JitClaw 现在还内置了一个 `jit` skill，并随包携带 `jit-cli` 二进制，因此启动后即可直接用于 JIT 后端工作流。
...
### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 JitClaw 在系统登录后自动启动。
...
预构建安装包通过项目的 OSS 发布通道分发，不再使用 GitHub Releases。
...
> Moonshot（Kimi）说明：JitClaw 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，JitClaw 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

JitClaw 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，JitClaw 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
- 当 JitClaw 代理处于关闭状态时，Gateway 的常规重启会保留已有的 Telegram 频道代理配置。
...
JitClaw 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
- JitClaw 基于 Electron，**单个应用实例出现多个系统进程是正常现象**（main/renderer/zygote/utility）。
...
- 点击窗口关闭按钮（`X`）默认只是最小化到托盘，并不会完全退出应用。请在托盘菜单中选择 **Quit JitClaw** 执行完整退出。
...
```bash
# 开发
pnpm run init             # 安装依赖并下载 uv 与 jit-cli
pnpm dev                  # 以热重载模式启动（若缺失会自动准备预装技能包）
pnpm run jit:download     # 下载当前平台的 jit-cli
pnpm run jit:download:all # 下载所有支持平台的 jit-cli
...
打包时内置 `uv` 默认优先走国内可用的 USTC 镜像，失败后再回退 GitHub Releases。如需覆盖默认镜像，可设置 `CLAWX_UV_DOWNLOAD_BASE_URL=https://your-oss.example.com/uv/{version}`。内置 `jit-cli` 二进制则从 `zjfjiayou/jit-cli` 的官方 GitHub Releases 下载，并写入 `resources/bin/<platform-arch>/`；默认跟随 latest release，如需锁定版本，可通过 `CLAWX_JIT_VERSION=<version>` 或 `zx scripts/download-bundled-jit.mjs --version=<version>` 覆盖。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 JitClaw 变得更好。
...
我们正在启动 JitClaw 合作伙伴计划，寻找能够帮助我们将 JitClaw 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，JitClaw 团队则提供完整的技术支持、定制开发与集成服务。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=zjfjiayou/JitClaw&type=Date" alt="Stars 历史图表" />
</p>
...
JitClaw 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
